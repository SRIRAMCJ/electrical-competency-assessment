...
const ASSETS = {
  bulb: '/BULB.fbx',
  battery: '/battery.fbx',
  ammeter: '/ampermeter.fbx',
  voltmeter: '/voltmeter.fbx',
  variableResistor: '/variable%20resistor.fbx'
} as const;
...
export default function InteractiveActivity({ onFinish }: { onFinish: (score: number) => void }) {
  ...
  useEffect(() => {
      ...
      if (round === 2) {
        ...
        for (const item of items) {
          if (item.url) {
            ...
            catch { /* keep assessment usable even if an asset fails */ }
          } else {
    ...
    load().catch(() => setNotice('Some 3D assets could not be loaded. Check the asset paths.'));
  ...
  return <div className="activityPage threeLab"><style>{css}</style>
    ...
    <main className="activityMain"><div className="activityIntro"><div><div className="eyebrow">STAGE 2 • 3D INTERACTIVE VALIDATION</div><h1>Electrical <span>Troubleshooting Lab</span></h1><p>Inspect the real authored 3D assets, rotate and zoom the equipment, then complete the task without relying on labels.</p></div><div className="activityProgress"><b>{round+1}/3</b><small>challenges</small></div></div>
      ...
      <section className="labGrid"><section className="labSceneCard"><div className="sceneToolbar"><div><b>3D Interactive Workbench</b><small>Drag to rotate • wheel to zoom • right-drag to pan</small></div><span className="modePill">AUTHORED 3D ASSETS</span></div><div className="threeViewport" ref={mount}/><div className="sceneHint"><span>🖱️ Drag = rotate</span><span>◉ Wheel = zoom</span><span>⇧ Right-drag = pan</span>{round===1&&<span>🔎 Click fuse shield = open / close</span>}{round===2&&<span>🎯 Click a tool = select</span>}</div>{notice&&<div className="labNotice">{notice}</div>}</section>
