...
const labels: Record<Terminal, string> = {
  bp: '+', bm: '−', ai: 'A', ao: 'A', lp: 'L', lm: 'L', rp: 'R', rm: 'R'
};
        ...
        <section className="circuitBoard" onDragOver={e => e.preventDefault()} onDrop={() => { if (dragged) { place(dragged); setDragged(null); } }}>
          <div className="blankBoard">
            ...
            <svg className="circuitWires" viewBox="0 0 1000 560" preserveAspectRatio="none" aria-hidden="true">
              {connections.map(([a,b], i) => { const pos: Record<Terminal,[number,number]> = { bp:[120,255],bm:[120,385],ai:[315,255],ao:[430,255],lp:[615,255],lm:[615,385],rp:[500,455],rm:[700,455] }; const [x1,y1]=pos[a], [x2,y2]=pos[b]; const mx=(x1+x2)/2; return <path key={i} d={`M ${x1} ${y1} C ${mx} ${y1-35}, ${mx} ${y2+35}, ${x2} ${y2}`} stroke={wireColor} strokeWidth="6" fill="none" strokeLinecap="round"/>; })}
            </svg>
